...
const AllMedia = (props) => {
  ...
  let categoryMedia = props?.dataCategory?.CategoryMedia
  let sousCategorie = props?.dataSousCategorie?.SousCategorie
  //   console.log(' categoryMedia ======>>>>>', sousCategorie)
...
export const getServerSideProps = async ({ query }) => {
  ...
  var CategoryMedia = null

  if (titleRouter === 'التلفزة الرقمية') {
    SousCategorie = await getItemsMenu(51)
    CategoryMedia = await getItemsMenu(50)
  }
  ...
  return {
    props: JSON.parse(
      JSON.stringify({
        MenuGlobal: MenuGlobal,
        dataLive: dataLive,
        dataCategory: CategoryMedia,
        dataSousCategorie: SousCategorie,
      })
    ),
  }
}
